...
from coreRunner.world import World
...
class API(object):
    ...
    def clear_client_debug(self):
        aa=self
        self.player_debug = {k: {'exceptions': [], 'logs': [], 'fatal_error': []} for k, _ in self.player_debug.items()}
    ...
    def get_visio_state(self):
        ...
        world_state.update({'debug': {k: v for k, v in self.player_debug.items()}})
    ...
    def go_to_floor(self, player, floor, elevator_id):
        elevator = [x for x in self.world.get_elevator_for(player) if x.id == elevator_id]
        if not elevator:
            return
        elevator = elevator[0]
    ...
    def set_elevator_to_passenger(self, player, passenger_id, elevator_id):
        elevator = [x for x in self.world.get_elevator_for(player) if x.id == elevator_id]
        passenger = [x for x in  self.world.get_passengers() if x.id == passenger_id ]
        if not passenger:
            return
